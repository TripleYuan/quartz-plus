...
public class HttpUtils {
   ...
   * 默认的RequestConfig：
   * <ul>
   *   <li>连接超时时间{@link #CONNECTION_REQUEST_TIMEOUT}</li>
   *   <li>读取响应数据超时时间{@link #SOCKET_TIMEOUT}</li>
   *   <li>从连接池中获取连接得超时时间{@link #CONNECTION_REQUEST_TIMEOUT}</li>
   * </ul>
   */
  ...
  /**
   * 保存用户自定义的RequestConfig，发送http请求前使用，请求完成后（包括异常完成），清除RequestConfig
   */
  ...
  public static String doDelete(String url) {
    ...
    try {
      ...
      // set RequestConfig
      Optional.ofNullable(getRequestConfig()).ifPresent(httpDelete::setConfig);
  ...
  public static String transferFiles(String url,
    ...
    try {
      ...
      // set RequestConfig
      Optional.ofNullable(getRequestConfig()).ifPresent(post::setConfig);
  ...
  /**
   * 设置 http request config，仅对下一次http请求有效
   */
  ...
  /**
   * 获取已设置http request config，如果未设置，返回默认的配置{@link HttpUtils#DEFAULT_REQUEST_CONFIG}
   */
  public static RequestConfig getRequestConfig() {
    return Optional.ofNullable(REQUEST_CONFIG_CONTEXT.get()).orElse(DEFAULT_REQUEST_CONFIG);
  }

  /**
   * 移除手动设置的http request config
   */
  ...
  /**
   * <ul>
   *     <li>HttpUtils会输出请求信息和返回信息的info日志，如果想禁用这类info日志，可调用{@link HttpUtils#disableNextHttpRequestInfoLog()}
   ...
   *     http请求。</li>
   *     <li>修改RequestConfig，调用{@link #setRequestConfig(RequestConfig)}，修改默认的RequestConfig {@link HttpUtils#DEFAULT_REQUEST_CONFIG }</li>
   * </ul>
   */
  static void 配置项() {
